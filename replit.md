...
- **File Upload**: Resumable, chunked uploads via the tus protocol (`@tus/server` + `@tus/file-store`) mounted at `/api/uploads/tus*`. Each chunk is its own short HTTP request, so transient drops only re-send the failing chunk and uploads survive page reloads / network changes. Auth is enforced at the Express layer and re-checked on every chunk: the uploader id and target project are bound to the upload at create time, so even an authenticated peer who learns an upload URL gets 403 on HEAD/PATCH/DELETE/finish. Multer is retained only for the legacy `/api/projects/:projectId/upload` endpoint used by non-UI integrations and comment attachments.
...
- **Whisper Backends**: Selectable via `OBVIU_WHISPER_BACKEND` env var on the spark. `faster_whisper` (default) uses an in-process WhisperModel with PyPI's `ctranslate2` — CPU-only on aarch64 because the prebuilt wheel ships no CUDA backend. `whisper_cpp` shells out to a CUDA-built `whisper-cli` binary for real GPU acceleration on GB10 (~10–20× realtime); the install is one-shot via `spark/build-whispercpp-cuda.sh`. Both backends emit the identical JSON shape so the app side is backend-agnostic. The whisper.cpp pipeline is ffmpeg→16kHz-mono-WAV→`whisper-cli --output-json-full`→parse→remap, and reuses the same `_JOB_LOCK` + queue + NFS save logic.
- **Video Processing**: FFmpeg-based video encoding with configurable timeouts for large file support (up to 2+ hour processing times). GPU encoding via NVIDIA NVENC (`h264_nvenc`) is auto-detected at startup — on hosts with a usable GPU (e.g. the Tesla T4) the quality + scrub passes encode on the GPU; hosts without one fall back to `libx264`. Override with `VIDEO_USE_NVENC=true|false` to force a specific encoder. The chosen encoder is logged at startup as `[Video] NVENC auto-detect: …`. The GPU pipeline uses **CPU decode + CPU scale + NVENC encode** rather than full `-hwaccel cuda + scale_cuda`, because (a) NVDEC can't decode every accepted input codec (ProRes, DNxHD, VP9 profile 2, 10-bit HEVC) and (b) `scale_cuda` doesn't auto-round to even dimensions, so portrait/non-16:9 sources can produce odd output (e.g. 405×720) which NVENC's NV12 format rejects. CPU decode + GPU encode keeps the expensive work on the T4 (~2× realtime) while accepting any input.
