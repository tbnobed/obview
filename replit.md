...
- **Whisper-on-Blackwell caveat**: CTranslate2 prebuilt wheels target CUDA 12, the Spark ships CUDA 13. If the first transcribe fails with a missing `libcublas.so.12`, install `libcublas-12-9 libcudnn9-cuda-12` (cu12 user-space is forward-compatible with the 580 driver) — see `spark/README.md` "Whisper on Blackwell" for the rebuild fallback.
