...
- **Video Processing**: FFmpeg-based video encoding with configurable timeouts for large file support (up to 2+ hour processing times). GPU encoding via NVIDIA NVENC (`h264_nvenc`) is auto-detected at startup — on hosts with a usable GPU (e.g. the DGX Spark) the quality + scrub passes encode on the GPU; hosts without one fall back to `libx264`. Override with `VIDEO_USE_NVENC=true|false` to force a specific encoder. The chosen encoder is logged at startup as `[Video] NVENC auto-detect: …`.
