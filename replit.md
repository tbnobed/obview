...
- **Spark LLM (separate from Whisper)**: Key moments and synopsis are generated by an LLM, **not** Whisper — Whisper is speech-to-text only. The LLM is `Qwen2.5-7B-Instruct` (Q4_K_M GGUF, `~/models/qwen2.5-7b-instruct.Q4_K_M.gguf` from `bartowski/Qwen2.5-7B-Instruct-GGUF`) served by `llama.cpp`'s `llama-server` (`/home/obtv-admin/llama.cpp/build/bin/llama-server --host 0.0.0.0 --port 8080 -ngl 99 -c 32768`), wrapped in a systemd unit so it auto-starts. The app reaches it via `LLAMA_API_URL=http://<spark-ip>:8080` (OpenAI-compatible `/v1/chat/completions`). This server is **not** in `spark/` — that directory is the Whisper FastAPI worker only. If you ever swap the model, update this line and `LLAMA_API_URL` (no app restart needed; `server/llm-client.ts` reads the env per-request).
