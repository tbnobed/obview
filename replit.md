...
- **Spark AI worker** (`spark/` in this repo): tiny FastAPI service that runs on the Spark via `setup.sh` (creates venv, installs deps, writes a systemd unit `obviu-spark-ai.service` bound to `192.168.100.1:7681` — DAC interface only). Endpoints: `GET /health` (liveness, GPU snapshot via `nvidia-smi`, NFS mount sanity including `isRdma`/`isNfs` flags), `GET /info`, `GET /probe?path=<rel>` (runs `ffprobe` on a file inside the shared mount, returns full JSON metadata; path-traversal-safe). Wired into the app's existing diagnostics by setting `SPARK_HOST=192.168.100.1`, `SPARK_PORT=7681`, `SPARK_DIAG_URL=http://192.168.100.1:7681/health` — no app code change needed; `probeSpark` already understands a JSON worker URL. Iteration 1 is plumbing only; real model endpoints (whisper-large transcription, CLIP, captioning) come next and will read media directly through the NFS-RDMA mount, no copy.
