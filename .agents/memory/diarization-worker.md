...
- ctranslate2 dlopens libcudnn_ops from pip's nvidia-cudnn-cu12 wheel, which the linker can't see inside the venv → worker ABRTs mid-transcription ("Unable to load any of {libcudnn_ops.so.9...}"). Unit must set LD_LIBRARY_PATH to the venv's nvidia/cudnn/lib + nvidia/cublas/lib (setup-local-gpu.sh now computes it); PYTHONUNBUFFERED=1 required or startup prints hide in the journal buffer.
