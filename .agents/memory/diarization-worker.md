...
- The same worker service (spark/service.py) deploys to both the DGX Spark (DAC-bound) and the app host's L4 (`setup-local-gpu.sh`, 127.0.0.1:7682, mount root = local uploads dir). The app switches targets purely via `SPARK_AI_URL` — no app code change.
